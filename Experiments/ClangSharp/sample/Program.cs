...
internal static unsafe class Program
{
    ...
    {
        var tilesetFilename = Marshal.StringToCoTaskMemUTF8(
        ...
        fixed (int* c = TCOD_CHARMAP_CP437.AsSpan())
            tcodTileset = TCOD_tileset_load(
                (sbyte*)tilesetFilename.ToPointer(),
                16,
                16,
                TCOD_CHARMAP_CP437.Length,
                c
            );
        Marshal.FreeCoTaskMem(tilesetFilename);
        ...
        var contextParams = new TCOD_ContextParams()
        {
            columns = 80,
            rows = 50,
            window_title = (sbyte*)titlePtr.ToPointer(),
            tileset = tcodTileset,
        };
