...
public unsafe class BspGeneratedTests
{
    ...
    public void BspTraversal_PreOrderMatchesExpectedStructure()
    {
        ...
        try
        {
            TCOD_bsp_split_once(root, 0, 0);
            var left = TCOD_bsp_left(root);
            Assert.False(left == null);

            TCOD_bsp_split_once(left, 0, 0);
            ...
            Assert.True(TCOD_bsp_traverse_in_order(root, &AlwaysContinue, null) != 0);
            Assert.True(TCOD_bsp_traverse_post_order(root, &AlwaysContinue, null) != 0);
            Assert.True(TCOD_bsp_traverse_level_order(root, &AlwaysContinue, null) != 0);
            Assert.True(TCOD_bsp_traverse_inverted_level_order(root, &AlwaysContinue, null) != 0);
        }
        finally
        {
    ...
    private static byte AlwaysContinue(TCOD_bsp_t* node, void* userData)
    {
        return 1;
    }

    [UnmanagedCallersOnly(CallConvs = new[] { typeof(CallConvCdecl) })]
    private static byte CollectNodePointers(TCOD_bsp_t* node, void* userData)
    {
